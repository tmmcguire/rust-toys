...
pub fn bisect_left<T : Ord>(a : &[T], x : T, lo : uint, hi : uint) -> uint {
    let mut lo = lo;
    let mut hi = hi;
    while lo < hi {
        let mid = (lo + hi) / 2;
...
pub fn bisect_left_ref<T : Ord>(a : &[T], x : &T, lo : uint, hi : uint) -> uint {
    let mut lo = lo;
    let mut hi = hi;
    while lo < hi {
        let mid = (lo + hi) / 2;
...
pub fn bisect_left_with_compare<T>(a : &[T], 
                                   x : &T, 
                                   lo : uint, 
                                   hi : uint,  
...
pub fn bisect_right<T : Ord>(a : &[T], x : T, lo : uint, hi : uint) -> uint {
    let mut lo = lo;
    let mut hi = hi;
    while lo < hi {
        let mid = (lo + hi) / 2;

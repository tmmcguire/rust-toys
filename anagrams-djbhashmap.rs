...
fn load_dictionary() -> HashMap<Vec<u8>,Vec<String>> {
    ...
    for line in bufferedFile.lines() {
        ...
        map.insert(Vec::from_fn(key.len(), |i| *key.get(i) as u8),
...
fn get_letters(s : &str) -> Vec<u8> {
    ...
    t.sort();
    return Vec::from_fn(t.len(), |i| *t.get(i) as u8);
}

fn search(letters : &[u8], dictionary : &HashMap<Vec<u8>,Vec<String>>) -> HashSet<String>
{
